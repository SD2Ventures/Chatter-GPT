...
create table if not exists mentions (
  ...
  domain text,                 -- e.g., subreddit, site, etc.
  url text,
  title text,
  body text,
  author_hash text,
...
create table if not exists events (
  id bigserial primary key,
  entity_id int not null references entities(id) on delete cascade,
  event_type text not null,    -- earnings, 8-K, guidance, breach, halt, m&a, etc.
  title text,
  summary text,
  url text,
  source text,                 -- sec, news, pr, exchange
  published_at timestamptz not null default now(),
  confidence numeric
);
create index if not exists idx_events_entity_time on events(entity_id, published_at desc);

-- Optional view for quick chatter score (safe even when empty)
